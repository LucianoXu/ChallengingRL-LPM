...
- **Final-experiment direction (designed in `docs/SPEC.md`, not yet run):** move to **sparse-reward MiniGrid** (Youssef's `minigrid_intrinsic_reward` repo) — a β (intrinsic-coefficient) sweep on RND with extrinsic/intrinsic reward tracked separately, a difficulty ladder (clean → noisy, easy → hard) to find where intrinsic methods finally win, and an LPM-vs-RND noise-robustness comparison on a noisy-MiniGrid variant. The course report is still to be written.
