...
- `LPM_exploration/.venv/` — uv-created Python 3.11 venv used for Noisy-MNIST, the Miniworld play tool, and the maze experiments. **Not committed** (gitignored). Includes: `torch`, `numpy`, `matplotlib`, `tqdm`, `python-mnist`, `jupyter`, `gymnasium`, `miniworld`, `pygame`, `Pillow`, `torchvision`, `pandas`, `pytest`, `sb3-contrib`.
